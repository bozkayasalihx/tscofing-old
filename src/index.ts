...
class Select {
    ...
    public start() {
        ...
        for (let opt = 0; opt < this.opts.options.length; opt++) {
            this.opts.options[opt] =
                this.opts.pointer + " " + this.opts.options[opt];

            this.opts.options[opt] += "\n";
            if (opt === 0) {
                this.input = 0;
                process.stdout.write(
                    this.color(this.opts.options[opt], this._color)
                );
            } else {
    ...
    private color(str: string, colorName = "yellow") {
        const colors = {
            yellow: [33, 89],
            blue: [34, 89],
            green: [32, 89],
